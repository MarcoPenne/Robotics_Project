...
if (clientID>-1)
    ...
    [returnCode]=sim.simxLoadScene(clientID,'scenes/1-dof_torque_control.ttt',1,sim.simx_opmode_blocking);
...
while seconds(datetime('now')-start) < period * 5
    ...
    feedforward = Y_matrix_1dof(position(t), acceleration(t)) * get_1dof_coefficients(7.3009, 0.3428, 0.1789);
    c = Kp*(position(t) - joint_position) + Kd*(velocity(t) - angular(1));
    u = c + feedforward;
    % Sending commands
